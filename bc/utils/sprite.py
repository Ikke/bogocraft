...
class AbstractGroup(object):
    ...
    def remove_internal(self, sprite):
        ...
        if r is not 0:
            self.removed_sprite_rects.append(r)
    ...
    def clear(self, surface, bgd):
        ...
        try:
            ...
            for r in self.removed_sprite_rects:
                bgd(surface, r)
...
class LayeredUpdates(AbstractGroup):
    ...
    def remove_internal(self, sprite):
        ...
        self.removed_sprite_rects.append(self.spritedict[sprite]) # dirty rect
        if hasattr(sprite, 'rect'):
            self.removed_sprite_rects.append(sprite.rect) # dirty rect
    ...
    def draw(self, surface):
        ...
        spritedict = self.spritedict
        surface_blit = surface.blit
        dirty = self.removed_sprite_rects
        self.removed_sprite_rects = []
        dirty_append = dirty.append
...
class LayeredDirty(LayeredUpdates):
    ...
    def find_dirty_rectangles(self, _clip):
        # 1. find dirty area on screen and put the rects into self.lostsprites
        ...
        for spr in self._spritelist:
            if 0 < spr.dirty:
                ...
                if spr.source_rect:
                    ...
                    _union_rect = Rect(spr.rect)

                _union_rect_collidelist = _union_rect.collidelist
                _union_rect_union_ip = _union_rect.union_ip
                i = _union_rect_collidelist(self.removed_sprite_rects)
                while -1 < i:
                    _union_rect_union_ip(self.removed_sprite_rects[i])
                    del self.removed_sprite_rects[i]
                    i = _union_rect_collidelist(self.removed_sprite_rects)
                self.removed_sprite_rects.append(_union_rect.clip(_clip))

                _union_rect = Rect(self.spritedict[spr])
                _union_rect_collidelist = _union_rect.collidelist
                _union_rect_union_ip = _union_rect.union_ip
                i = _union_rect_collidelist(self.removed_sprite_rects)
                while -1 < i:
    ...
    def draw_updated_sprites(self, surface):
        # 2. draw
        for spr in self._spritelist:
            if not spr.is_dirty():
                if spr._visible:
                    ...
                    for idx in _spr_rect.collidelistall(self.removed_sprite_rects):
                        # clip
                        clip = _spr_rect_clip(self.removed_sprite_rects[idx])
                        surface.blit(spr.image, clip,\
                            (clip[0] - _spr_rect[0],\
                             clip[1] - _spr_rect[1],\
                             clip[2],\
    ...
    def _update_dirty_sprites(self, _bgd, _clip, surface):

        self.find_dirty_rectangles(_clip)# clear using background

        if _bgd is not None:
            for rec in self.removed_sprite_rects:
                surface.blit(_bgd, rec, rec)

        self.draw_updated_sprites(surface)

        return list(self.removed_sprite_rects)

    def _draw_full_surface(self, _bgd, _clip, surface):
        if _bgd is not None:
            surface.blit(_bgd, (0, 0))
        for spr in self._spritelist:
            if spr._visible:
                self.spritedict[spr] = surface.blit(spr.image, spr.rect, spr.source_rect, spr.blendmode)
        _ret = [Rect(_clip)] # return only the part of the screen changed
        return _ret

    def _update_use_update(self, start_time, end_time):
    ...
    def draw(self, surface, bgd=None):
        ...
        if self._use_update: # dirty rects mode
            return_value = self._update_dirty_sprites(_bgd, _clip, surface)
        else: # flip, full screen mode
            return_value = self._draw_full_surface(_bgd, _clip, surface)

        # timing for switching modes
        # how to find a good treshold? it depends on the hardware it runs on
        self._update_use_update(start_time, get_ticks())
    ...
    def repaint_rect(self, screen_rect):
        ...
        """
        self.removed_sprite_rects.append(screen_rect.clip(self._clip))
